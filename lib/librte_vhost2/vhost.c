...
static void msg_handler_start_all_vqs(struct vhost_dev *vdev, uint16_t starting_idx);
...
vhost_dev_ops_complete(struct vhost_dev *vdev, int rc)
{
	...
	if (vdev->removed && !vdev->deleted && vdev->op_pending_cnt == 0)
...
static void
_msg_handler_start_all_vqs_cpl(struct vhost_dev *vdev, int rc, void *ctx)
{
	struct vhost_vq *vq = ctx;
	...
	assert(vq->q.idx < VHOST_MAX_VIRTQUEUES - 1);
	msg_handler_start_all_vqs(vdev, vq->q.idx + 1);
}

static void
msg_handler_start_all_vqs(struct vhost_dev *vdev, uint16_t starting_idx)
{
	struct vhost_vq *vq;
	uint16_t vq_idx;
	...
	for (vq_idx = starting_idx; vq_idx < VHOST_MAX_VIRTQUEUES; vq_idx++) {
		...
		if (!vdev->ops->queue_start) {
			_msg_handler_start_all_vqs_cpl(vdev, 0, vq);
			continue;
		}

		vhost_dev_set_ops_cb(vdev, _msg_handler_start_all_vqs_cpl, vq);
...
{
	/* to prevent the device destroy callback from calling
	 * this function again, we set the deleted flag.
	 */
	vdev->deleted = true;
...
_handle_msg(struct vhost_dev *vdev)
{
	...
	case VHOST_USER_GET_PROTOCOL_FEATURES:
		if (!(vdev->features &
				(1ULL << VHOST_USER_F_PROTOCOL_FEATURES))) {
			RTE_LOG(ERR, VHOST_CONFIG,
				"VHOST_USER_F_PROTOCOL_FEATURES not negotiated\n");
			ret = -1;
			break;
		}
