...
{
	struct VhostUserMsg *msg = &conn->msg;
	int ret;

	ret = read_fd_message(conn->fd, (char *)msg,
		offsetof(struct VhostUserMsg, payload.u64),
...
static void
_vhost_user_post_device_destroy(struct vhost_dev *vdev, int rc, void *ctx __rte_unused)
{
	struct vhost_user_connection *conn = container_of(vdev,
			struct vhost_user_connection, vdev);
...
vhost_user_destroy_connection(struct vhost_user_connection *conn)
{
	...
	if (vsocket->ops->device_destroy) {
		...
		vsocket->ops->device_destroy(&conn->vdev.dev);
		return;
	}

	_vhost_user_post_device_destroy(&conn->vdev, 0, NULL);
}

static void vhost_user_read_cb(int connfd __rte_unused, void *ctx);
static int vhost_user_handle_msg(struct vhost_dev *vdev, struct VhostUserMsg *msg);
static void vhost_user_msg_cpl(struct vhost_dev *vdev, struct VhostUserMsg *msg);

static void
_vhost_user_new_connection(struct vhost_dev *vdev, int rc, void *ctx __rte_unused)
{
	struct vhost_user_connection *conn = container_of(vdev,
			struct vhost_user_connection, vdev);
	...
	conn->vdev.transport_ops = &vhost_dev_user_ops;
	...
	if (rc < 0) {
		...
		if (conn->vsocket->ops->device_destroy) {
			vhost_dev_set_ops_cb(&conn->vdev, _vhost_user_free_connection, NULL);
			conn->vsocket->ops->device_destroy(&conn->vdev.dev);
			return;
		}
...
static int
vhost_user_tgt_register(const char *path, uint64_t flags, void *ctx __rte_unused,
...
static bool
vhost_user_memory_changed(struct VhostUserMemory *new,
...
static int
vhost_user_set_mem_table(struct vhost_dev *vdev, struct VhostUserMsg *msg)
{
	...
	mem = rte_zmalloc("vhost-user-mem-table", sizeof(struct vhost_user_memory) +
	...
	vhost_mem = rte_zmalloc("vhost-mem-table", sizeof(struct rte_vhost2_memory) +
...
static int
vhost_user_handle_msg(struct vhost_dev *vdev, struct VhostUserMsg *msg)
{
	switch (msg->type) {
...
static void
vhost_user_msg_cpl(struct vhost_dev *vdev, struct VhostUserMsg *msg __rte_unused)
{
	struct vhost_user_connection *conn = container_of(vdev,
			struct vhost_user_connection, vdev);
	int ret;

	assert(conn->queued_msgs > 0);
	conn->queued_msgs--;
...
static int
vhost_user_send_reply(struct vhost_dev *vdev, struct VhostUserMsg *msg)
{
	struct vhost_user_connection *conn = container_of(vdev,
			struct vhost_user_connection, vdev);

	return send_fd_message(conn->fd, (char *)msg,
			offsetof(VhostUserMsg, payload.u64) + msg->size,
			NULL, 0);
}
