...
fdset_init(struct fdset *pfdset)
{
	...
	if (fcntl(pfdset->u.writefd, F_SETFL, O_NONBLOCK))
		fprintf(stderr,
			"couldn't set O_NONBLOCK for pipe writefd (%d) - %s\n",
			pfdset->u.writefd, strerror(errno));
...
static void
_fdset_deinit_cb(int fd __rte_unused, int rc, void *ctx)
{
...
{
	return fdset_del(pfdset, pfdset->u.readfd, _fdset_deinit_cb);
}
...
_fdset_event_dispatch(void *arg)
{
	...
	do {
		...
		for (i = 0; i < polled_fds_num; i++) {
			...
			if (pfdentry->rcb &&
					pfd->revents & (POLLIN | FDPOLLERR))
				pfdentry->rcb(pfdentry->fd, pfdentry->ctx);
			if (pfdentry->wcb &&
					pfd->revents & (POLLOUT | FDPOLLERR))
