 ...
 * virtqueues are ready to be processed, features have changed at runtime, etc.
 */
...
#ifndef VHOST_ACCESS_RO
#define VHOST_ACCESS_RO      0x1
#endif

#ifndef VHOST_ACCESS_WO
#define VHOST_ACCESS_WO      0x2
#endif

#ifndef VHOST_ACCESS_RW
 ...
 * \c rte_vhost2_tgt_ops callbacks. This struct is published just
 * for the inline vdev functions to access their data directly.
 */
struct rte_vhost2_dev {
	const struct vhost_transport_ops *transport; /**< internal ops */
...
/**
 * Device/queue related callbacks, all optional. Each callback is asynchronous
 * unless explicitly specified, and needs to be completed by calling
 * \c rte_vhost2_dev_op_complete. Most callbacks will terminate the connection
 * if completed with a non-zero status. rte_vhost2 will call only one callback
 * per device at a time. That is, if a device is awaiting its \c
 * rte_vhost2_dev_op_complete then no other callbacks will be called for that
 * particular device.
 *
 * All callbacks for devices made by a single target are called from a single
 * thread.
 */
struct rte_vhost2_tgt_ops {
	...
	void (*device_create)(struct rte_vhost2_dev *vdev,
			const char *trtype, const char *trid);
	/**
	...
	*/
	...
	/**
	* Features have changed at runtime. This is called at least once during
	...
	* `queue_stop` is called. If completed with a non-zero status,
	* rte_vhost2 may attempt to start this queue later.
	*/
	void (*queue_start)(struct rte_vhost2_dev *vdev,
	...
	* device/vq state. If completed with a non-zero status, rte_vhost2 will
	* call this callback again.
	*/
	void (*queue_stop)(struct rte_vhost2_dev *vdev,
			struct rte_vhost2_vq *vq);
	/**
	 * Device disconnected. All queues are guaranteed to be stopped by now.
	 * If completed with a non-zero status, rte_vhost2 will call this
	 * callback again.
	 */
	 ...
	 * be available until \c rte_vhost2_tgt_cb_complete is called. The
	 * return code handling upon completion is specific to the msg id.
	 */
	void (*custom_msg)(struct rte_vhost2_dev *vdev,
			struct rte_vhost2_vq *vq,
			const char *id, void *ctx);

	/** Interrupt handler, synchronous. */
	void (*queue_kick)(struct rte_vhost2_dev *vdev,
			struct rte_vhost2_vq *vq);
	/**
	 * Full device config read, synchronous. Return 0 if `len` bytes of
	 * `config` have been successfully set, or negative errno otherwise.
	 */
	int (*get_config)(struct rte_vhost2_dev *vdev, uint8_t *config,
			uint32_t len);
	/**
	 * Device config changed by the driver, synchronous. `type` indicates
	 * the reason of change. Return 0 if the config was successfully
	 * processed and is valid, or negative errno otherwise.
	 */
 ...
 * Unix domain socket polling local connections or connect to a PCI vhost
 * device and install an interrupt handler. A separate `rte_vhost2_dev`
 * struct will be created for each communication endpoint.
 *
 ...
 * states.
 * \param features supported vhost-user features. To be negotiated with the
 * driver ones. rte_vhost2 will append a couple of generic feature bits
 * which are required by the vhost-user spec. TODO list these features here
...
/**
 * Unregisters a vhost target asynchronously. All active queues will be stopped
 * and all devices destroyed.
 *
 * This function is thread-safe.
 *
 * \param cb_fn callback to be called on finish. It'll be called from the same
 * thread that calls \c rte_vhost2_tgt_ops for that target.
 ...
 * \param vq virtqueue. Must be started.
 */
void rte_vhost2_dev_call(struct rte_vhost2_dev *vdev,
		struct rte_vhost2_vq *vq);
