...
struct vhost_unix_memory {
	uint32_t nregions;
	struct vhost_unix_mem_region regions[];
};

struct vhost_unix_socket {
	char *path;
	int socket_fd;
	uint64_t features;
	const struct rte_vhost2_tgt_ops *ops;

	TAILQ_HEAD(, vhost_unix_connection) conn_list;
...
struct vhost_unix_connection {
	struct vhost_unix_socket *vsocket;
	struct vhost_dev vdev;

	struct vhost_unix_memory *mem;
...
struct vhost_unix {
	TAILQ_HEAD(, vhost_unix_socket) vsockets;
	struct fdset fdset;
};
...
static struct vhost_unix vhost_unix = {
	.vsockets = TAILQ_HEAD_INITIALIZER(vhost_unix.vsockets),
};
...
/* return bytes# of read on success or negative val on failure. */
static int
read_vhost_message(struct vhost_unix_connection *conn)
{
...
_vhost_unix_free_connection(void *arg)
{
	...
	struct vhost_unix_socket *vsocket = conn->vsocket;
...
static void
_vhost_unix_connfd_del_cb(int fd __rte_unused, int rc, void *ctx)
{
	struct vhost_unix_connection *conn = ctx;

	assert(rc == 0);
	vhost_dev_destroy(&conn->vdev, _vhost_unix_free_connection, conn);
}
...
vhost_unix_destroy_connection(struct vhost_unix_connection *conn)
{
	...
	if (rc < 0)
		RTE_LOG(ERR, VHOST_CONFIG,
			"failed to remove fd %d from vhost server fdset - %s\n",
			conn->fd, strerror(-rc));
}

static void vhost_unix_read_cb(int connfd __rte_unused, void *ctx);

static void
_vhost_unix_new_connection_cpl(int fd __rte_unused, int rc, void *ctx)
{
	struct vhost_unix_connection *conn = ctx;
	...
	if (rc) {
		...
		vhost_dev_destroy(&conn->vdev,
				_vhost_unix_free_connection, conn);
		return;
	}
...
static void
_vhost_unix_new_connection(struct vhost_dev *vdev, int rc,
		void *ctx __rte_unused)
{
	struct vhost_unix_connection *conn = container_of(vdev,
			struct vhost_unix_connection, vdev);
	...
	rc = fdset_add(&vhost_unix.fdset, conn->fd, vhost_unix_read_cb,
			NULL, conn, _vhost_unix_new_connection_cpl);
	if (rc < 0) {
		RTE_LOG(ERR, VHOST_CONFIG,
...
static void
vhost_unix_add_connection(int fd, struct vhost_unix_socket *vsocket)
{
	...
	vhost_dev_init(&conn->vdev, conn->vsocket->features,
			&vhost_unix_transport,
			&vhost_dev_unix_ops, conn->vsocket->ops);
	conn->fd = fd;
	...
	if (vsocket->ops->device_create) {
		vhost_dev_set_ops_cb(&conn->vdev,
				_vhost_unix_new_connection, NULL);
		vsocket->ops->device_create(&conn->vdev.dev,
				vhost_unix_transport.type, vsocket->path);
		return;
	}

	_vhost_unix_new_connection(&conn->vdev, 0, NULL);
}

/* call back when there is new vhost-user connection from client  */
static void
vhost_unix_server_new_connection(int fd, void *ctx)
{
	...
	vhost_unix_add_connection(fd, vsocket);
}

static int
create_unix_socket(struct vhost_unix_socket *vsocket)
{
...
static void
_vhost_unix_start_server_cpl(int fd, int rc, void *ctx)
{
	struct vhost_unix_socket *vsocket = ctx;

	if (rc) {
		RTE_LOG(ERR, VHOST_CONFIG,
			"failed to add listen fd %d to vhost server fdset - %s\n",
			fd, strerror(-rc));
		close(fd);
		vsocket->socket_fd = -1;
...
static int
vhost_unix_start_server(struct vhost_unix_socket *vsocket)
{
	int ret;
	...
	ret = fdset_add(&vhost_unix.fdset, fd, vhost_unix_server_new_connection,
		  NULL, vsocket, _vhost_unix_start_server_cpl);
	if (ret < 0) {
		RTE_LOG(ERR, VHOST_CONFIG,
...
static int
vhost_unix_tgt_register(const char *path, uint64_t flags,
		void *ctx __rte_unused,
		...
		uint64_t features)
{
	...
	vsocket = calloc(1, sizeof(struct vhost_unix_socket));
	if (!vsocket) {
	...
	if (TAILQ_EMPTY(&vhost_unix.vsockets))
		fdset_init(&vhost_unix.fdset);

	TAILQ_INSERT_TAIL(&vhost_unix.vsockets, vsocket, tailq);

	rc = vhost_unix_start_server(vsocket);
	if (rc) {
		TAILQ_REMOVE(&vhost_unix.vsockets, vsocket, tailq);
		close(vsocket->socket_fd);
		free(vsocket->path);
		...
		void (*cb_fn)(void *arg), void *cb_ctx)
{
	...
	if (rc < 0)
		RTE_LOG(ERR, VHOST_CONFIG,
			"failed to remove server fd %d from vhost fdset - %s\n",
			vsocket->socket_fd, strerror(-rc));

	return rc;
}

static void
_vhost_unix_dev_op_complete(int fd __rte_unused, void *ctx)
{
	struct vhost_unix_connection *conn = ctx;
...
static void
vhost_unix_dev_op_complete(struct rte_vhost2_dev *_vdev, int rc)
{
	...
	rc = fdset_notify(&vhost_unix.fdset, _vhost_unix_dev_op_complete, conn);
	if (rc < 0)
		RTE_LOG(ERR, VHOST_CONFIG,
			"failed to notify vhost fdset about op complete - %s\n",
			strerror(-rc));
}

static void
vhost_unix_dev_call(struct rte_vhost2_dev *vdev __rte_unused,
...
static void
vhost_unix_dev_cfg_call(struct rte_vhost2_dev *_vdev)
{
	struct vhost_dev *vdev = container_of(_vdev,
			struct vhost_dev, dev);
	struct vhost_unix_connection *conn = container_of(vdev,
...
static bool
vhost_unix_memory_changed(struct vhost_user_msg_memory *new,
		     struct vhost_unix_memory *old)
{
	uint32_t i;
	...
	for (i = 0; i < new->nregions; ++i) {
		...
		struct vhost_unix_mem_region *old_r = &old->regions[i];
...
static int
vhost_unix_set_mem_table(struct vhost_dev *vdev, struct vhost_user_msg *msg)
{
	...
	struct vhost_unix_mem_region *reg;
	...
	if (conn->mem && !vhost_unix_memory_changed(&memory, conn->mem)) {
	...
	mem = rte_zmalloc("vhost-unix-mem-table",
		sizeof(struct vhost_unix_memory) +
		sizeof(struct vhost_unix_mem_region) * memory.nregions, 0);
	if (mem == NULL) {
		RTE_LOG(ERR, VHOST_CONFIG,
			"failed to allocate memory for vhost unix mem\n");
		return -1;
	}
...
static int
vhost_unix_set_slave_req_fd(struct vhost_dev *vdev, struct vhost_user_msg *msg)
{
	struct vhost_unix_connection *conn = container_of(vdev,
			struct vhost_unix_connection, vdev);
...
static int
vhost_unix_handle_msg(struct vhost_dev *vdev, struct vhost_user_msg *msg)
{
	switch (msg->type) {
	case VHOST_USER_SET_MEM_TABLE:
		return vhost_unix_set_mem_table(vdev, msg);
	case VHOST_USER_SET_SLAVE_REQ_FD:
		return vhost_unix_set_slave_req_fd(vdev, msg);
	default:
		return 1;
	}
}
...
static void
_vhost_unix_device_init_cpl(struct vhost_dev *vdev __rte_unused,
		int rc, void *ctx)
{
	struct vhost_unix_connection *conn = ctx;
	...
	fdset_enable(&vhost_unix.fdset, conn->fd, true);
}

static void
vhost_unix_msg_cpl(struct vhost_dev *vdev, int rc,
	...
	case VHOST_USER_SET_MEM_TABLE:
		if (conn->mem && vdev->ops->device_init) {
			vhost_dev_set_ops_cb(vdev,
					_vhost_unix_device_init_cpl, conn);
...
static int
vhost_unix_send_reply(struct vhost_dev *vdev, struct vhost_user_msg *msg)
{
	struct vhost_unix_connection *conn = container_of(vdev,
			struct vhost_unix_connection, vdev);
	int ret;
...
static void
vhost_unix_read_cb(int connfd __rte_unused, void *ctx)
{
	struct vhost_unix_connection *conn = ctx;
	int ret;

	fdset_enable(&vhost_unix.fdset, conn->fd, false);
...
static struct vhost_dev_ops vhost_dev_unix_ops = {
	.handle_msg = vhost_unix_handle_msg,
	.msg_cpl = vhost_unix_msg_cpl,
	.send_reply = vhost_unix_send_reply,
};
...
static struct vhost_transport_ops vhost_unix_transport = {
	...
	.dev_cfg_call = vhost_unix_dev_cfg_call,
};

VHOST_TRANSPORT_REGISTER(vhost_unix_transport);
